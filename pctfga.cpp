...
int main(int argc, char *argv[])
{
    ...
    strcpy(dir,"Solucoes");
    sprintf(slv,"..\\Solucoes\\tamPop-%i qtdCrz-%i tamElt-%i prcMut-%i", TAM_POP, QTD_CRZ, TAM_ELT, PRC_MUT);
    mkdir(slv);
    lerInstancia(arq);
    montarRede();
    //testEntRede();
    ...
    {
        printf("\n\n>>> Resolvendo a instancia %s ALFA = %d e BETA = %d TAM_POP = %i QTD_CRZ = %i TAM_ELT = %i PRC_MUT = %i - rodada %d\n\n",INST,ALFA,BETA,TAM_POP, QTD_CRZ, TAM_ELT, PRC_MUT, r);
        execGA();
        sol = populacao[0];
...
void crossover(){
    int a, b;
    for (int i = TAM_POP - (TAM_POP * (1 - ((float)QTD_CRZ/100))); i < TAM_POP; i++){
        do{
            a = rand() % (int)(TAM_POP * ((float)TAM_ELT/100));
